layui.use(['upload','element','form','layer','table','laytpl'],function(){
	...
	var form = layui.form,
    	...
    	upload.render({
    	    ...
    	    accept: 'file',
    	    exts:'xlsx|xls',
    	    url:$("#ctx").attr("value") + '/replaceStr/upload.ht',	//  /CrawlerSystem
    	    method: 'post',
    	    auto: false,
    	    bindAction: '#test9',
    	    done: function(res){
    ...
    var tableIns = table.render({
        ...
        url :$("#ctx").attr("value") + '/replaceStr/getList.ht',
        cellMinWidth : 95,
        page : true,
        height : "full-95",
        limits : [10,15,20,25],
        ...
        cols : [[
            ...
            {field: 'replaceGroup', title: 'replace_group', align:'center',minWidth:100},
            {field: 'replaceStr', title: 'replace_str', align:'center',minWidth:180},
            {field: 'replaceVal', title: 'replace_val', align:'center',minWidth:180},
  ...
  //添加
    function addFiled(edit){
    	...
    	if(edit){
            ...
            $(".filedId").val(edit.id);
            $(".replaceGroup").val(edit.replaceGroup);
            $(".replaceStr").val(edit.replaceStr);
            $(".replaceVal").val(edit.replaceVal);
    		$(".addFlied").hide();
    		$(".editFlied").show();
        }
    ...
    form.on("submit(addFlied)",function(data){
       ...
       $.post($("#ctx").attr("value") + '/replaceStr/addRecord.ht',{
        	replaceGroup : $(".replaceGroup").val(),
        	replaceStr : $(".replaceStr").val(),
        	replaceVal : $(".replaceVal").val(),
        },function(res){
        	tableIns.reload();
        	top.layer.msg("添加成功！");
    	    $(".replaceGroup").val("");
    ...
    form.on("submit(editFlied)",function(data){
        ...
        //弹出loading
       var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
       $.post($("#ctx").attr("value") + '/replaceStr/editRecord.ht',{
    	   id : $(".filedId").val(),
    	   replaceGroup : $(".replaceGroup").val(), 
    	   replaceStr : $(".replaceStr").val(), 
    	   replaceVal : $(".replaceVal").val(), 
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.get($("#ctx").attr("value") + "/replaceStr/delAllRecord.ht",{
    ...
    table.on('tool(filedList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
        	...
        	layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                $.get($("#ctx").attr("value") + "/replaceStr/delRecord.ht",{
                	id : data.id
                },function(data){
                	top.layer.msg("删除成功！");
                	tableIns.reload();
  ...
  function getRemark(){
  	$.ajax({
  		//url: '../../../json/remark.json',
  		  ...
  		  type: 'post',
  		  data: {"remarkId" : "replace_str"},
  		  dataType: 'json',
  		  timeout: 2000,
  		  success: function (data, status) {
  		    var remarks = '<li>replace_str  - 说明</li>';

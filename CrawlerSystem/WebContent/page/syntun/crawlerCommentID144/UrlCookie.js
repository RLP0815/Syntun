layui.use(['upload','element','form','layer','table','laytpl'],function(){
	...
	var form = layui.form,
    	...
    	upload.render({
    	    ...
    	    accept: 'file',
    	    exts:'xlsx|xls',
    	    url:$("#ctx").attr("value") + '/crawlerCommentJD144UrlCookie/upload.ht',	//  /CrawlerSystem
    	    method: 'post',
    	    auto: false,
    	    bindAction: '#test9',
    	    done: function(res){
    ...
    var tableIns = table.render({
        ...
        url :$("#ctx").attr("value") + '/crawlerCommentJD144UrlCookie/getList.ht',
        cellMinWidth : 95,
        page : true,
        height : "full-95",
        limits : [10,15,20,25],
        ...
        cols : [[
            ...
            {field: 'cookieStr', title: 'cookie_str', align:'center',minWidth:100},
  ...
  //添加
    function addFiled(edit){
    	$(".addFlied").show();
    	$(".editFlied").hide();
    	var title = "添加filter_url";
    	if(edit){
    		title = "编辑filter_url";
            
            $(".filedId").val(edit.id);  
            $(".urlGroup").val(edit.urlGroup);
            $(".cookieStr").val(edit.cookieStr);
    ...
    form.on("submit(addFlied)",function(data){
       ...
       $.post($("#ctx").attr("value") + '/crawlerCommentJD144UrlCookie/addRecord.ht',{
        	urlGroup : $(".urlGroup").val(),
        	cookieStr : $(".cookieStr").val(),
        	sortId : $(".sortId").val(),
        },function(res){
        	tableIns.reload();
        	top.layer.msg("添加成功！");
    ...
    form.on("submit(editFlied)",function(data){
        //弹出loading
       var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
       $.post($("#ctx").attr("value") + '/crawlerCommentJD144UrlCookie/editRecord.ht',{
    	   id : $(".filedId").val(),
    	   urlGroup : $(".urlGroup").val(), 
    	   cookieStr : $(".cookieStr").val(), 
    	   sortId : $(".sortId").val(), 
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.get($("#ctx").attr("value") + "/crawlerCommentJD144UrlCookie/delAllRecord.ht",{
    ...
    table.on('tool(filedList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
        	...
        	layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                $.get($("#ctx").attr("value") + "/crawlerCommentJD144UrlCookie/delRecord.ht",{
                	id : data.id
                },function(data){
                	top.layer.msg("删除成功！");
                	tableIns.reload();
  ...
  function getRemark(){
  	$.ajax({
  		//url: '../../../json/remark.json',
  		  ...
  		  type: 'post',
  		  data: {"remarkId" : "crawlerCommentJD144UrlCookie"},
  		  dataType: 'json',
  		  timeout: 2000,
  		  success: function (data, status) {
  		    var remarks = '<li>url_cookie  - 说明</li>';

layui.use(['upload','laydate','element','form','layer','table','laytpl'],function(){
	...
	var form = layui.form,
    	...
    	upload.render({
    	    ...
    	    accept: 'file',
    	    exts:'xlsx|xls',
    	    url:$("#ctx").attr("value") + '/crawler144InitUrlList/upload.ht',	//  /CrawlerSystem
    	    method: 'post',
    	    auto: false,
    	    bindAction: '#test9',
    	    done: function(res){
    ...
    var tableIns = table.render({
        ...
        url :$("#ctx").attr("value") + '/crawler144InitUrlList/getList.ht',
        cellMinWidth : 95,
        page : true,
        height : "full-95",
        limits : [10,15,20,25],
  ...
  //添加
    ...
    form.on("submit(addFlied)",function(data){
       ...
       $.post($("#ctx").attr("value") + '/crawler144InitUrlList/addRecord.ht',{
    	    urlStr : $(".urlStr").val(),
    ...
    form.on("submit(editFlied)",function(data){
        ...
        //弹出loading
       var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
       $.post($("#ctx").attr("value") + '/crawler144InitUrlList/editRecord.ht',{
    	   id : $(".filedId").val(),
    	   urlStr : $(".urlStr").val(),
    	   urlGroup : $(".urlGroup").val(), 
    	   patternTable : $(".patternTable").val(), 
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.get($("#ctx").attr("value") + "/crawler144InitUrlList/delAllRecord.ht",{
    ...
    table.on('tool(filedList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
        	...
        	layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                $.get($("#ctx").attr("value") + "/crawler144InitUrlList/delRecord.ht",{
                	id : data.id
                },function(data){
                	top.layer.msg("删除成功！");
                	tableIns.reload();
  ...
  function getRemark(){
  	$.ajax({
  		//url: '../../../json/remark.json',
  		  ...
  		  type: 'post',
  		  data: {"remarkId" : "crawler144InitUrlList"},
  		  dataType: 'json',
  		  timeout: 2000,

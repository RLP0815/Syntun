...
public class UrlLimitThread implements Runnable {
	...
	public void run() {
		while (true) {
			...
			if (urlLimit.getInsertStatus()
					&& System.currentTimeMillis() - urlLimit.getUserInsertTime() >= urlLimit.getIntervalTime()
					&& userUrlList.size() > 0) {
				synchronized (userUrlList) {
	...
	private synchronized void selectUrlInfo() {
		String sql = "SELECT b.* FROM (SELECT id FROM `url_status` WHERE is_parse_over=" + urlLimit.getStatus()
				+ " and try_num<" + UrlSupervise.maxTryGetNum + " and get_time<date_sub(now(), interval "
				+ UrlSupervise.tryGetUrlTime + " minute) LIMIT 0, 100) a JOIN `url_list` b ON(a.id=b.id)";

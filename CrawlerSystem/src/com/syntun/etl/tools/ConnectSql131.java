...
public class ConnectSql131 {
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = ConnectSql131.bacthStmt.indexOf(stmt);
		if (s == -1)
			ConnectSql131.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = ConnectSql131.bacthStmt.indexOf(stmt);
		if (s != -1)
			ConnectSql131.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (ConnectSql131.bacthStmt.size() > 0) {
			sNum += ConnectSql131.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (ConnectSql131.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < ConnectSql131.sqlCache.size(); i++)
					stmt.addBatch(ConnectSql131.sqlCache.get(i));
			...
			ConnectSql131.sqlCache.clear();
			ConnectSql131.sqlCache = new LinkedList<String>();
		}
		ConnectSql131.sqlCache.add(sql);

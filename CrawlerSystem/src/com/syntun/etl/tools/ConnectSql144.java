...
public class ConnectSql144 {
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = ConnectSql144.bacthStmt.indexOf(stmt);
		if (s == -1)
			ConnectSql144.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = ConnectSql144.bacthStmt.indexOf(stmt);
		if (s != -1)
			ConnectSql144.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (ConnectSql144.bacthStmt.size() > 0) {
			sNum += ConnectSql144.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (ConnectSql144.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < ConnectSql144.sqlCache.size(); i++)
					stmt.addBatch(ConnectSql144.sqlCache.get(i));
			...
			ConnectSql144.sqlCache.clear();
			ConnectSql144.sqlCache = new LinkedList<String>();
		}
		ConnectSql144.sqlCache.add(sql);

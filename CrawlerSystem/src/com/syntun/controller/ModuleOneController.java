...
public class ModuleOneController {
    ...
    @SysLogger(modelName = "QC一校对数据", methodType = "getCheckResult")
    public Map getCheckResult(String checkMethod, String database, String tableName, String column1, String column2, String method) {
 		...
 		try {
			...
			String userName = request.getSession().getAttribute("userName").toString();
 			...
 			String outText = "数据库："+database+"  表："+tableName+"  查询列："+column1+"  被查询列："+column2;
			...
			n++;
 			...
 			for(String key : mapSet.keySet()){
 				...
 				if((checkMethod.equals("qc_btn1") && st.size() > 1)
 						|| checkMethod.equals("qc_btn2")){
 					for (String s : st) {  
 						 out += "\t" + key + "\t\t\t" + s + "\n";
 						 
 						 sheet.addCell(new Label(0, n, key)); 
    ...
    @SysLogger(modelName = "QC一批量校对数据", methodType = "getCheckResults")
			...
			WritableSheet sheet = workbook.createSheet("Sheet", 0);
 			...
 			for(int i=0; i<database.split(",").length; i++){
 				String url = "jdbc:sqlserver://192.168.0.15:1433;DatabaseName="+database.split(",")[i];
 	 			...
 	 			String outText = i+1 + "、  数据库："+database.split(",")[i]+"  表："+tableName.split(",")[i]+"  查询列："+column1.split(",")[i]+"  被查询列："+column2.split(",")[i];

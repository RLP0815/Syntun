...
<!-- 			<input type="text" class="layui-input oldFiled" id="ip" lay-verify="required" > -->
	        <select name="modules" lay-verify="required" lay-search="" id="ip">
<!-- 	          <option value="1">layer</option>
	...
	<div class="layui-form-item layui-row layui-col-xs12">
		<div class="layui-input-block">
			<a class="layui-btn layui-btn-sm" id="queryFile">查看文件</a>
			<a class="layui-btn layui-btn-sm" id="saveFile" style="display: none">保存文件</a>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	$("#queryFile").click(function(){
		...
		var index = top.layer.msg('数据加载中，请稍候',{icon: 16,time:false,shade:0.8});
   	  	$.ajax({
	  		  url: $("#ctx").attr("value") + '/command/readData',
			  type: 'post',
			  data: {
				  "ip":ip,
				  "path":filePath,
	...
	layui.use(['element','form','layer','table','laytpl'],function(){
	    ...
	    var tableIns = table.render({
	        elem: '#filedList',
	        url :$("#ctx").attr("value") + '/user/getHistoryInfo?type=2',
	        cellMinWidth : 95,
	        page : true,
	        height : "full-68",
	        limits : [10,15,20,25],

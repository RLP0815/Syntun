...
public class Url {
		...
		try {
			if (dataStr != null && !dataStr.equals("")) {
				...
				for (int i = 0; i < dataArr.length; i++) {
					...
					try {
						this.savePageInfo.put(dataInfo[0],
								dataInfo[1].replaceAll(this.replaceUrlAnd, "&").replaceAll(this.replaceUrldengyu, "="));
					} catch (Exception e) {
	...
	public String getDataStr() {
		...
		if (!this.getData().isEmpty()) {
			int i = 0;
			for (Iterator<Entry<String, String>> iter = this.getData().entrySet().iterator(); iter.hasNext(); i++) {
				Entry<String, String> e = iter.next();
				if (i != 0)
					urlDataStr += "&";
				urlDataStr += e.getKey() + "="
						+ e.getValue().replaceAll("&", this.replaceUrlAnd).replaceAll("=", this.replaceUrldengyu);
			}
		}
		return urlDataStr;
	}
	...
	public String getCookie() {
		String cookieStr = "";
		int i = 0;
		for (Iterator<Entry<String, String>> iter = this.saveCookieHm.entrySet().iterator(); iter.hasNext(); i++) {
			Entry<String, String> e = iter.next();
			if (i != 0)
				cookieStr += "; ";

layui.use(['upload','laydate','element','form','layer','table','laytpl'],function(){
	...
	upload.render({
	    ...
	    accept: 'file',
	    exts:'xlsx|xls',
	    url:$("#ctx").attr("value") + '/crawlerTmall144DataTableFiledList/upload.ht',	//  /CrawlerSystem
	    method: 'post',
	    auto: false,
	    bindAction: '#test9',
	    done: function(res){
    ...
    var tableIns = table.render({
        elem: '#filedList',
        url :$("#ctx").attr("value") + '/crawlerTmall144DataTableFiledList/getList.ht',
        cellMinWidth : 95,
        page : true,
        height : "full-95",
        limits : [10,15,20,25],
  ...
  //添加
    ...
    form.on("submit(addFlied)",function(data){
       ...
       $.post($("#ctx").attr("value") + '/crawlerTmall144DataTableFiledList/addRecord.ht',{
    	    tableId : $(".tableId").val(),
    ...
    form.on("submit(editFlied)",function(data){
        ...
        //弹出loading
       var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
       $.post($("#ctx").attr("value") + '/crawlerTmall144DataTableFiledList/editRecord.ht',{
    	   filedId : $(".filedId").val(),
    	   tableId : $(".tableId").val(),
    	   filedName : $(".filedName").val(), 
    	   filedDataType : $(".filedDataType").val(), 
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.get($("#ctx").attr("value") + "/crawlerTmall144DataTableFiledList/delAllRecord.ht",{
    ...
    table.on('tool(filedList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
        	...
        	layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                $.get($("#ctx").attr("value") + "/crawlerTmall144DataTableFiledList/delRecord.ht",{
                	id : data.filedId
                },function(data){
                	top.layer.msg("删除成功！");
                	tableIns.reload();
  ...
  function getRemark(){
  	$.ajax({
  		//url: '../../../json/remark.json',
  		  ...
  		  type: 'post',
  		  data: {"remarkId" : "crawlerTmall144DataTableFiledList"},
  		  dataType: 'json',
  		  timeout: 2000,

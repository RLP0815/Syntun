layui.use(['laydate','element','form','layer','table','laytpl'],function(){
  ...
  //天猫特殊促销
    ...
    //立即执行
    form.on("submit(tmallExcel)",function(data){
    	...
    	if(excelPath.endsWith(".xlsx") || excelPath.endsWith(".xls")){
	       ...
	       $.post($("#ctx").attr("value") + '/replaceFiled/mateExcel.ht',{
	        	excelPath : $(".excelPath").val(),  
	        },function(res){
	        	//top.layer.msg("添加成功！");
	        	$(".excelPath").val("");
	        	$("#tmallFiled").dialog("close"); 
	        })
	        
	        return false;
    	}else{
    		layer.msg("文件名不是excel格式,请重新选择");
    		return false;
    	}

...
function demo(ip){
					  ...
					  //console.log(JSON.stringify(serverInfo));
					    ...
					    if(serverInfo.length>0){
							...
							for (var i = 0; i < serverInfo.length; i++) {
								...
								var state = serverInfo[i].state;
								var logfilestate = serverInfo[i].logfilestate;
								var state1 = "state1"+serverInfo[i].id;
								var state2 = "state2"+serverInfo[i].id;
								var state3 = "state3"+serverInfo[i].id;
								htm += "<div>\n"+
								...
								if(logfilestate=='1'){
									htm += "<input type=\"button\" id="+state3+" onclick=\"serverLogFileYujing('"+serverInfo[i].id+"','"+state3+"','0')\" value=\"已预警\" style=\"background-color: red;\">\n"+
							           '</div>\n';
								}else{
									htm += "<input type=\"button\" id="+state3+" onclick=\"serverLogFileYujing('"+serverInfo[i].id+"','"+state3+"','1')\" value=\"未预警\" style=\"background-color: green;\">\n"+
							           '</div>\n';
								}
							}
							$("#serverDiv").empty();
	...
	//
	function serverLogFileYujing(tableid,id,logstate){
   	  	$.ajax({
	  		  url: $("#ctx").attr("value") + '/command/serverLogFileYujing',
			  type: 'post',
			  data: {
				  "tableid":tableid,
				  "logstate":logstate,

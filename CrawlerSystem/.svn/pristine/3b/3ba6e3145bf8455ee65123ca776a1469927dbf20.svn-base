...
public class HBaseUtils {
	...
	public Result getOneRow(String tableName, String rowKey) {
		...
		System.out.println("单条查询时间:"+(endTime-statrTime));
		return rsResult;
	}
	...
	public List<Result> getRowFilter(String tableName, String value) {
		HTableInterface table = null;
		List<Result> list = null;
		try {
			table = hTablePool.getTable(tableName);
	...
	public List<Result> getColumn(String tableName, String family, String column,String value,String tableNameValue) {
		...
		try {
			table = hTablePool.getTable(tableName);
			Scan scan = new Scan();
			scan.setStartRow(value.getBytes());
			//product+表名模糊查询
...
/*				for (Cell cell : rsResult.rawCells()){
		...
		System.out.println("________________Start - end = "+(endTime-startTime) +"_ list Size=__"+list.size());
		return list;
	}

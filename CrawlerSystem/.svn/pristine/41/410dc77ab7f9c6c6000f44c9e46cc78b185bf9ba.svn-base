layui.use(['upload','laydate','element','form','layer','table','laytpl'],function(){
	...
	var form = layui.form,
    	...
    	upload.render({
    	    ...
    	    accept: 'file',
    	    exts:'xlsx|xls',
    	    url:$("#ctx").attr("value") + '/crawlerCommentJD144PurlList/upload.ht',	//  /CrawlerSystem
    	    method: 'post',
    	    auto: false,
    	    bindAction: '#test9',
    	    done: function(res){
    ...
    var tableIns = table.render({
        ...
        url :$("#ctx").attr("value") + '/crawlerCommentJD144PurlList/getList.ht',
        cellMinWidth : 95,
        page : true,
        height : "full-95",
        limits : [10,15,20,25],
        ...
        cols : [[
            ...
            {field: 'purlStr', title: 'purl_str', align:"center", minWidth:180},
            {field: 'params', title: 'params', align:'center',minWidth:120},
            {field: 'exeParams', title: 'exe_params', align:'center',minWidth:100},
            {field: 'patternId', title: 'pattern_id', align:'center',minWidth:120},
            {field: 'parentPurlId', title: 'parent_purl_id', align:'center',minWidth:120},
            ...
            {field: 'writeTime', title: 'write_time', align:'center',minWidth:180},
            {field: 'remark', title: 'remark', align:'center',minWidth:180},
  ...
  //添加
    function addFiled(edit){
    	...
    	$(".addFlied").show();
    	$(".editFlied").hide();
    	var title = "添加purl_list";
    	if(edit){
    		title = "编辑purl_list";
            
            $(".filedId").val(edit.id);  
            $(".urlGroup").val(edit.urlGroup);
        	$(".purlStr").val(edit.purlStr);
            $(".params").val(edit.params);
            $(".exeParams").val(edit.exeParams);
            $(".patternId").val(edit.patternId);
            $(".parentPurlId").val(edit.parentPurlId);
    ...
    form.on("submit(addFlied)",function(data){
       ...
       $.post($("#ctx").attr("value") + '/crawlerCommentJD144PurlList/addRecord.ht',{
        	urlGroup : $(".urlGroup").val(),
    	    purlStr : $(".purlStr").val(),
        	params : $(".params").val(),
        	exeParams : $(".exeParams").val(),
        	patternId : $(".patternId").val(),
        	parentPurlId : $(".parentPurlId").val(),
        	sortId : $(".sortId").val(),
    ...
    form.on("submit(editFlied)",function(data){
        //弹出loading
       var index = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
       $.post($("#ctx").attr("value") + '/crawlerCommentJD144PurlList/editRecord.ht',{
    	   id : $(".filedId").val(),
    	   urlGroup : $(".urlGroup").val(), 
    	   purlStr : $(".purlStr").val(),
    	   params : $(".params").val(), 
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.get($("#ctx").attr("value") + "/crawlerCommentJD144PurlList/delAllRecord.ht",{
    ...
    table.on('tool(filedList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
        	...
        	layer.confirm('确定删除此记录？',{icon:3, title:'提示信息'},function(index){
                $.get($("#ctx").attr("value") + "/crawlerCommentJD144PurlList/delRecord.ht",{
                	id : data.id
                },function(data){
                	top.layer.msg("删除成功！");
                	tableIns.reload();
  ...
  function getRemark(){
  	$.ajax({
  		//url: '../../../json/remark.json',
  		  ...
  		  type: 'post',
  		  data: {"remarkId" : "crawlerCommentJD144PurlList"},
  		  dataType: 'json',
  		  timeout: 2000,
  		  success: function (data, status) {
  		    var remarks = '<li>purl_list  - 说明</li>';

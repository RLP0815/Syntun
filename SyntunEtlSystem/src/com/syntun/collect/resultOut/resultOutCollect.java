...
public class resultOutCollect extends Configured implements Tool {
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new resultOutCollect(), args);
		System.exit(res);
	}
	...
	public int run(String[] arg0) throws Exception {
		...
		conf.set("mapreduce.job.reduces", "120");
		...
		//String input = "/tmp/SyntunCommentResult/";
		String input = "/tmp/SyntunCommentResult48W/";
		String input2 = "/tmp/SyntunPriceAvg/";
		//String output = "/tmp/SyntunCommentResultOut/" + beginEnd;
		String output = "/tmp/SyntunCommentResultOut/48W/" + beginEnd;
		...
		Job job = new Job(conf, "commentResultOut48W job");
		job.setJarByClass(resultOutCollect.class);
		...
		String beginYear = beginEnd.split("_")[0].split("-")[0];
		String endYear = beginEnd.split("_")[1].split("-")[0];
		if(beginYear.equals(endYear)){
			CombineFileInputFormat.addInputPath(job, new Path(input2 + beginYear + "/*"));
		}else{
			CombineFileInputFormat.addInputPath(job, new Path(input2 + beginYear+"_"+endYear + "/*"));
		}
		//CombineFileInputFormat.addInputPath(job, new Path(input2 + "2014_2015_2016/*"));

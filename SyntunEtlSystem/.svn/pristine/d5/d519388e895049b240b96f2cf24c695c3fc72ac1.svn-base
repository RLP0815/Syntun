...
public class priceOutCollect extends Configured implements Tool {
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new priceOutCollect(), args);
		System.exit(res);
	}
	...
	public int run(String[] arg0) throws Exception {
		...
		String output = "/tmp/SyntunPriceOut/" + beginEnd;
		FileTools.deleteFile(output,conf);
		
		conf.set("year", beginEnd);
		...
		Job job = new Job(conf, "priceOut job");
		job.setJarByClass(priceOutCollect.class);

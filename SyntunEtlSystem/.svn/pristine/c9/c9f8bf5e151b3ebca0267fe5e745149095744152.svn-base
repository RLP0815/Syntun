...
public class ConnectSqlServer13 {
	...
	public static String sqlDbName = "comment";
	public static String sqlHost = "192.168.0.20";
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = ConnectSqlServer13.bacthStmt.indexOf(stmt);
		if (s == -1)
			ConnectSqlServer13.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = ConnectSqlServer13.bacthStmt.indexOf(stmt);
		if (s != -1)
			ConnectSqlServer13.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (ConnectSqlServer13.bacthStmt.size() > 0) {
			sNum += ConnectSqlServer13.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (ConnectSqlServer13.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < ConnectSqlServer13.sqlCache.size(); i++)
					stmt.addBatch(ConnectSqlServer13.sqlCache.get(i));
			...
			ConnectSqlServer13.sqlCache.clear();
			ConnectSqlServer13.sqlCache = new LinkedList<String>();
		}
		ConnectSqlServer13.sqlCache.add(sql);

...
public class categoryGYCollect extends Configured implements Tool {
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new categoryGYCollect(), args);
		System.exit(res);
	}
...
//		}
		...
		conf.set("mapreduce.reduce.memory.mb", "28672");//4096、8192
		conf.set("mapreduce.reduce.java.opts", "-Xmx28672M");
		// map 阶段缓存大小，默认100
		...
		conf.set("mapreduce.reduce.shuffle.parallelcopies", "8");
		...
		String beginEnd = arg0[0];//GYOut
		String input = "/tmp/SyntunCommentResultOut/Category/grade/*";
		...
		Job job = new Job(conf, "categoryGYOut job");
		job.setJarByClass(categoryGYCollect.class);

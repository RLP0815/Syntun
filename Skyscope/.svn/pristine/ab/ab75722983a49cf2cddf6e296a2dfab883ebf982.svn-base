...
//	public static String sqlDbName = "wgdata_VIP";
//	public static String sqlHost = "192.168.0.124:3306";
	
	public static String sqlName = "xitong";
	public static String sqlPassWord = "xitong001";
	public static String sqlDbName = "xitong";
	public static String sqlHost = "192.168.0.132:3306";// 原来124
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = ConnectSql132.bacthStmt.indexOf(stmt);
		if (s == -1)
			ConnectSql132.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = ConnectSql132.bacthStmt.indexOf(stmt);
		if (s != -1)
			ConnectSql132.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (ConnectSql132.bacthStmt.size() > 0) {
			sNum += ConnectSql132.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (ConnectSql132.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < ConnectSql132.sqlCache.size(); i++)
					stmt.addBatch(ConnectSql132.sqlCache.get(i));
			...
			ConnectSql132.sqlCache.clear();
			ConnectSql132.sqlCache = new LinkedList<String>();
		}
		ConnectSql132.sqlCache.add(sql);

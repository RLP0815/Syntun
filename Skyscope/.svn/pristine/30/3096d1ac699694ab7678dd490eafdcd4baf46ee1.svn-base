...
//	public static String sqlHost = "192.168.0.124:3306";
	...
	public static String sqlPassWord = "syntun-000";
	public static String sqlDbName = "wgdata_VIP";
	public static String sqlHost = "192.168.0.131:3306";// 原来124
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = ConnectSql124.bacthStmt.indexOf(stmt);
		if (s == -1)
			ConnectSql124.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = ConnectSql124.bacthStmt.indexOf(stmt);
		if (s != -1)
			ConnectSql124.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (ConnectSql124.bacthStmt.size() > 0) {
			sNum += ConnectSql124.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (ConnectSql124.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < ConnectSql124.sqlCache.size(); i++)
					stmt.addBatch(ConnectSql124.sqlCache.get(i));
			...
			ConnectSql124.sqlCache.clear();
			ConnectSql124.sqlCache = new LinkedList<String>();
		}
		ConnectSql124.sqlCache.add(sql);

...
public class ConnectSql138 {
	private static final int CONN_TIME_OUT = 288000;

	public static String sqlName = "syntun";
	public static String sqlPassWord = "syntun";
	public static String sqlDbName = "skyscope_online";
	public static String sqlHost = "192.168.0.138:3306";
...
//	public static String sqlHost = "localhost:3306";
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = ConnectSql138.bacthStmt.indexOf(stmt);
		if (s == -1)
			ConnectSql138.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = ConnectSql138.bacthStmt.indexOf(stmt);
		if (s != -1)
			ConnectSql138.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (ConnectSql138.bacthStmt.size() > 0) {
			sNum += ConnectSql138.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (ConnectSql138.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < ConnectSql138.sqlCache.size(); i++)
					stmt.addBatch(ConnectSql138.sqlCache.get(i));
			...
			ConnectSql138.sqlCache.clear();
			ConnectSql138.sqlCache = new LinkedList<String>();
		}
		ConnectSql138.sqlCache.add(sql);

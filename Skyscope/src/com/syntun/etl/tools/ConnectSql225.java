...
public class ConnectSql225 {
	private static final int CONN_TIME_OUT = 288000;

	public static String sqlName = "peihaitao";
	public static String sqlPassWord = "pht13@25";
	public static String sqlDbName = "syntun_v2";
	public static String sqlHost = "192.168.0.225:3306";
...
//	public static String sqlHost = "localhost:3306";
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = ConnectSql225.bacthStmt.indexOf(stmt);
		if (s == -1)
			ConnectSql225.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = ConnectSql225.bacthStmt.indexOf(stmt);
		if (s != -1)
			ConnectSql225.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (ConnectSql225.bacthStmt.size() > 0) {
			sNum += ConnectSql225.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (ConnectSql225.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < ConnectSql225.sqlCache.size(); i++)
					stmt.addBatch(ConnectSql225.sqlCache.get(i));
			...
			ConnectSql225.sqlCache.clear();
			ConnectSql225.sqlCache = new LinkedList<String>();
		}
		ConnectSql225.sqlCache.add(sql);

...
//	public static String sqlHost = "localhost:3306";
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = ConnectSql22.bacthStmt.indexOf(stmt);
		if (s == -1)
			ConnectSql22.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = ConnectSql22.bacthStmt.indexOf(stmt);
		if (s != -1)
			ConnectSql22.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (ConnectSql22.bacthStmt.size() > 0) {
			sNum += ConnectSql22.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (ConnectSql22.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < ConnectSql22.sqlCache.size(); i++)
					stmt.addBatch(ConnectSql22.sqlCache.get(i));
			...
			ConnectSql22.sqlCache.clear();
			ConnectSql22.sqlCache = new LinkedList<String>();
		}
		ConnectSql22.sqlCache.add(sql);

...
public class ConnectSql73 {
	...
	public static String sqlDbName = "syntun_comment_no";
	public static String sqlHost = "192.168.0.73:3306";
...
//	public static String sqlHost = "localhost:3306";
	...
	public static synchronized void addStmt(Statement stmt) {
		int s = ConnectSql73.bacthStmt.indexOf(stmt);
		if (s == -1)
			ConnectSql73.bacthStmt.add(stmt);
	}

	public static synchronized void removeStmt(Statement stmt) {
		int s = ConnectSql73.bacthStmt.indexOf(stmt);
		if (s != -1)
			ConnectSql73.bacthStmt.remove(s);
	}

	public static synchronized int exeAllBacth() throws SQLException {
		int sNum = 0;
		while (ConnectSql73.bacthStmt.size() > 0) {
			sNum += ConnectSql73.bacthStmt.remove(0).executeBatch().length;
		}
		return sNum;
	}

	public static synchronized void cacheInserSql(String sql, Statement stmt) {
		if (ConnectSql73.sqlCache.size() >= 50) {
			try {
				for (int i = 0; i < ConnectSql73.sqlCache.size(); i++)
					stmt.addBatch(ConnectSql73.sqlCache.get(i));
			...
			ConnectSql73.sqlCache.clear();
			ConnectSql73.sqlCache = new LinkedList<String>();
		}
		ConnectSql73.sqlCache.add(sql);
